...
- `/civil-legal-mexico:matter-workspace new <slug>` — crear un nuevo espacio de trabajo, ejecutar una entrevista corta de ingreso, escribir `matter.md`
- `/civil-legal-mexico:matter-workspace list` — listar asuntos con estado y bandera de activo
- `/civil-legal-mexico:matter-workspace switch <slug>` — establecer el asunto activo
- `/civil-legal-mexico:matter-workspace close <slug>` — archivar un asunto (mover a `~/.claude/plugins/config/claude-for-legal/civil-legal-mexico/matters/_archived/`, nunca eliminar)
- `/civil-legal-mexico:matter-workspace none` — desactivar cualquier asunto activo, trabajar solo a nivel de práctica

## Instrucciones

1. Leer `~/.claude/plugins/config/claude-for-legal/civil-legal-mexico/CLAUDE.md` — confirmar que la sección `## Espacios de trabajo por asunto` esté poblada. Si `Enabled` es `✗`, indicar al usuario: "Los espacios de trabajo por asunto están desactivados — estás configurado como práctica in-house con un solo cliente, así que el plugin funciona automáticamente desde el contexto a nivel de práctica. Si en realidad trabajas con múltiples clientes, ejecuta `/civil-legal-mexico:cold-start-interview --redo` y selecciona una configuración de práctica privada. De lo contrario, no necesitas `/civil-legal-mexico:matter-workspace` en absoluto." No marcar error — el estado desactivado es el esperado para usuarios in-house.
2. Usar el flujo de trabajo descrito abajo.
3. Despachar según el primer token de `$ARGUMENTS`:
   - `new` → ejecutar la entrevista de ingreso, escribir `~/.claude/plugins/config/claude-for-legal/civil-legal-mexico/matters/<slug>/matter.md`, inicializar `history.md` y `notes.md`.
   - `list` → enumerar `~/.claude/plugins/config/claude-for-legal/civil-legal-mexico/matters/*/matter.md`, imprimir una tabla, marcar el asunto activo.
   - `switch` → actualizar la línea `Active matter:` en el CLAUDE.md a nivel de práctica.
   - `close` → mover `~/.claude/plugins/config/claude-for-legal/civil-legal-mexico/matters/<slug>/` a `~/.claude/plugins/config/claude-for-legal/civil-legal-mexico/matters/_archived/<slug>/`, registrar la fecha de cierre en `history.md`.
...
Los abogados de práctica múltiple (práctica privada — despacho individual, despacho mediano, despacho grande, notaría) trabajan en muchos asuntos. El contexto de uno no debe filtrarse a otro. Este skill es la capa delgada de gestión de archivos que hace eso posible.

**Estado predeterminado: desactivado.** Los usuarios in-house nunca ven esto — trabajan solo a nivel de práctica. Los espacios de trabajo por asunto se activan en el cold-start para usuarios de práctica privada, o editando `## Espacios de trabajo por asunto` en el CLAUDE.md a nivel de práctica. Si `Enabled` es `✗`, este skill no se ejecuta; `/civil-legal-mexico:matter-workspace` explica el estado desactivado y sugiere `/civil-legal-mexico:cold-start-interview --redo` para usuarios que realmente necesiten aislamiento por asunto.
...
```
~/.claude/plugins/config/claude-for-legal/civil-legal-mexico/
...
Los slugs son en minúsculas con guiones. Ejemplos: `lopez-arrendamiento-local-2026`, `garcia-promesa-terreno`, `sucesion-hernandez`.
...
2. Ejecutar la entrevista de ingreso:
   ...
   - **Tipo de asunto** (leer el perfil de práctica del plugin para categorías típicas; para civil-legal-mexico: arrendamiento lado arrendador | arrendamiento lado arrendatario | compraventa o promesa de inmueble | garantía (hipoteca, fianza, obligado solidario) | sucesión | responsabilidad civil | convenio familiar | otro)
   - **Entidad federativa del asunto** (ubicación del inmueble o lugar de cumplimiento; si difiere de la entidad principal del perfil, los skills citan el código de esta entidad)
...
6. **No** cambiar automáticamente al nuevo asunto. Preguntar: "¿Deseas cambiar a `<slug>` ahora? (`/civil-legal-mexico:matter-workspace switch <slug>`)"
...
1. Confirmar que `matters/<slug>/matter.md` exista. Si no, ofrecer `/civil-legal-mexico:matter-workspace new <slug>`.
...
[Entidad donde se ubica el inmueble o se cumple la obligación. Si difiere de la entidad principal del perfil de práctica, esta prevalece para citar.]

## Tipo de asunto

[arrendamiento lado arrendador | arrendamiento lado arrendatario | compraventa o promesa de inmueble | garantía | sucesión | responsabilidad civil | convenio familiar | otro — con justificación en una línea]
...
- [ej., "Depósito: el cliente acepta dos meses, no el estándar de la casa de uno."]
- [ej., "Tono: preservar la relación — el arrendador es cliente recurrente del despacho."]
- [ej., "Inmueble en Querétaro aunque el despacho opera en Jalisco: citar el Código Civil de Querétaro con `[VERIFICAR]`."]
...
[Cualquier contexto inicial que valga la pena preservar más allá de matter.md — ej., "Abierto en respuesta a borrador de contrato de arrendamiento recibido de [contraparte]."]
```
...
Cuando está en `on`, un skill puede leer archivos entre carpetas de asuntos solo cuando el usuario lo pide explícitamente (ej., "compara nuestra posición sobre depósito en garantía en los últimos cinco arrendamientos comerciales"). Incluso cuando está en `on`, el predeterminado es cargar solo el asunto activo a menos que el usuario pida una vista cross-matter.
