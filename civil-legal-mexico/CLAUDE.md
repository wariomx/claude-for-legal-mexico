...
La configuración específica del usuario para este plugin se encuentra en una ruta independiente de la versión que sobrevive a las actualizaciones del plugin:
  ...
  ~/.claude/plugins/config/claude-for-legal/civil-legal-mexico/CLAUDE.md

Reglas para cada skill, comando y agente en este plugin:
1. LEER la configuración en este orden (resolución local → global):
   a. LOCAL: .claude-legal/civil-legal-mexico/CLAUDE.md en el directorio de trabajo actual — si existe, es el perfil de este cliente/proyecto.
   b. GLOBAL: ~/.claude/plugins/config/claude-for-legal/civil-legal-mexico/CLAUDE.md — fallback cuando no hay config local.
   Si ninguno existe o aún tiene [PLACEHOLDER], DETENERSE y pedir cold-start-interview.
2. Si el archivo activo no existe o aún contiene marcadores [PLACEHOLDER], DETENERSE antes de realizar trabajo sustantivo. Decir: "Este plugin necesita configuración antes de poder darte resultados útiles. Ejecuta /civil-legal-mexico:cold-start-interview — toma entre 10 y 15 minutos y todos los comandos de este plugin dependen de ella. Sin esta configuración, los resultados serán genéricos y podrían no corresponder a tu práctica real." NO continuar con configuración placeholder o por defecto. Los únicos skills que funcionan sin configuración son /civil-legal-mexico:cold-start-interview y cualquier flag --check-integrations.
...
4. En la primera ejecución después de una actualización del plugin, si existe un CLAUDE.md ya configurado en la ruta antigua de caché
   (~/.claude/plugins/cache/claude-for-legal/civil-legal-mexico/<version>/CLAUDE.md para cualquier versión)
...
*Generado por cold-start el [FECHA]. Módulos activos: [Arrendamiento]*
*Si `[PLACEHOLDER]`, ejecuta `/civil-legal-mexico:cold-start-interview`.*

## Resolución de configuración

Los skills de este plugin buscan el perfil de práctica en este orden:

1. **Local (proyecto):** `.claude-legal/civil-legal-mexico/CLAUDE.md` en el directorio de trabajo actual — para aislamiento por cliente en despachos con múltiples clientes.
2. **Global (usuario):** `~/.claude/plugins/config/claude-for-legal/civil-legal-mexico/CLAUDE.md` — fallback para uso personal o de cliente único.

**Para crear config de cliente local:** ejecuta `/conectores-legal-mexico:setup-completo --local` (o `/civil-legal-mexico:cold-start-interview --local`) desde la carpeta del proyecto de ese cliente. **`.claude-legal/` debe estar en `.gitignore`** — contiene datos del cliente que no deben versionarse.
...
**Perfil civil:** [PLACEHOLDER — despacho civilista / notaría (apoyo jurídico) / jurídico interno con cartera inmobiliaria / administrador de inmuebles / persona física con asesoría]
...
**Escalamiento:** [PLACEHOLDER — socio responsable, despacho externo, notario de confianza, o Director Jurídico]
...
*El derecho civil es estatal. Cada skill lee esta sección antes de citar un solo artículo. Ver la regla dura `## Entidad antes de artículo` más abajo.*

**Entidad principal:** [PLACEHOLDER — entidad federativa donde se ubican la mayoría de los inmuebles o asuntos]
**Entidades adicionales:** [PLACEHOLDER — lista, o "ninguna"]
...
|---|---|---|---|---|
| Jalisco *(fila fija: entidad verificada del plugin; conservar aunque no sea tu entidad principal)* | Código Civil del Estado de Jalisco | ✓ `skills/revision-arrendamiento/references/ccj-jalisco-arrendamiento.md` (arrendamiento) | [PLACEHOLDER — código local / CNPCF con fecha de declaratoria] `[VERIFICAR]` | Registro Público de la Propiedad y de Comercio de Jalisco |
| [PLACEHOLDER — tu entidad principal o adicional, si no es Jalisco] | [PLACEHOLDER — nombre oficial del código civil de esa entidad] | ✗ — citas con `[VERIFICAR]` | [PLACEHOLDER] `[VERIFICAR]` | [PLACEHOLDER — registro público de la propiedad de esa entidad] |

*Solo Jalisco tiene tabla de artículos verificada en esta versión. Si tu entidad principal es Jalisco, la fila fija ya te cubre y la fila placeholder se elimina en cold-start. Para cualquier otra entidad, los skills conservan el checklist y citan con `[VERIFICAR: <código> art. <n>]`; ofrecer buscar el código vía LegalDataHunter (OrdenJuridicoEstatal) cuando el conector responda.*

---

**Modo discreto para entregables dirigidos a clientes y contrapartes.** Cuando un skill produce un entregable que será leído por una audiencia no jurídica o externa — una alerta al cliente, un memorándum para el cliente o el socio, una carta a la contraparte, un marcado de cambios para la contraparte, una carta de requerimiento, un convenio para firma — suprimir la narración interna. Específicamente:
...
El entregable debe leerse como si lo hubiera redactado el socio responsable del despacho. Los metacomentarios van en una nota del revisor arriba del encabezado o en un mensaje separado, no dentro del documento.

## Integraciones disponibles

| Integración | Estado | Alternativa si no está disponible |
|---|---|---|
| LegalDataHunter (códigos civiles estatales vía OrdenJuridicoEstatal, SCJN, DOF) | [✓ / ✗] | Solo Jalisco-arrendamiento queda verificado por la referencia local; el resto se cita `[VERIFICAR]` y el usuario pega el texto del código |
| CJJ — Poder Judicial de Jalisco (boletín y Portal Ciudadano) | [✓ / ✗] | Sin seguimiento de expedientes; el usuario reporta actuaciones manualmente |
| Registro Público de la Propiedad (consulta de folio real, gravámenes) | [✓ / ✗ — portal estatal, sin MCP] | El usuario pega el certificado de libertad de gravámenes; los skills lo leen como documento |
| Almacenamiento de documentos (Google Drive, SharePoint, Box, iManage) | [✓ / ✗] | Lee rutas locales; sin búsqueda entre sistemas |
| Slack | [✓ / ✗] | Los informes se emiten solo como archivos; sin resúmenes en canal |

*Re-verificar: `/civil-legal-mexico:cold-start-interview --check-integrations`*
...
*Retirar el encabezado de entregables dirigidos al exterior (contratos para firma, documentos presentados ante autoridad o notario, cartas a la contraparte, respuestas) — ver las instrucciones del skill específico. Los contratos firmados y los documentos protocolizados nunca se marcan como confidenciales; solo las notas de redacción y análisis anexos llevan esa marca.*
...
## Entidad antes de artículo — regla dura civil

El derecho civil mexicano son 32 códigos civiles estatales más el Código Civil Federal como supletorio. La numeración, los plazos máximos, los umbrales de inscripción, los derechos de preferencia y las reglas de orden público **difieren entre entidades**. Un artículo citado con el número correcto de otra entidad es un error que parece correcto.

1. **Ningún skill cita un artículo sin haber fijado la entidad.** La entidad se determina por la ubicación del inmueble (derechos reales, arrendamiento), el lugar de celebración o cumplimiento (obligaciones), o el último domicilio del autor de la sucesión. Si el asunto no la revela, preguntar. Nunca asumir la entidad principal del perfil.
2. **Referencia verificada o `[VERIFICAR]`.** Solo las entidades con archivo de referencia en `skills/<skill>/references/` se citan con `[settled — last confirmed <fecha>]`. Todo lo demás lleva `[VERIFICAR: <código> art. <n>]`, aunque el número "se vea bien".
3. **El CCF nunca se hace pasar por código estatal.** Se cita solo como supletorio o comparativo, y se dice expresamente.
4. **Materia procesal.** La vigencia del Código Nacional de Procedimientos Civiles y Familiares es por entidad y por declaratoria. Toda afirmación sobre vía o plazo procesal lleva `[VERIFICAR: estatus CNPCF en <entidad>]` salvo confirmación en la sesión.
5. **Ante duda entre dos plazos, el más corto es el operativo.** Se reporta el corto como regla de trabajo y el largo como `[review]`.
...
- Sugerir un skill estructurado si uno haría mejor trabajo: "Esta es una respuesta rápida. Si quieres el marco completo, ejecuta `/civil-legal-mexico:[skill relevante]`."

Si el perfil de práctica no está configurado: "Puedo darte una respuesta general, pero este plugin da respuestas mucho mejores una vez configurado a tu práctica — ejecuta `/civil-legal-mexico:cold-start-interview` (inicio rápido de 2 minutos o configuración completa de 10 minutos)." Luego dar la respuesta general de todos modos, marcada como no configurada.
...
Los marcos, pruebas, leyes y procedimientos por defecto de este plugin se basan en el derecho civil mexicano: el código civil de la entidad federativa aplicable, el Código Civil Federal como supletorio, y el código procesal civil vigente en esa entidad (local o CNPCF). Dos niveles de reconocimiento: (a) **otra entidad federativa** — el marco es mexicano pero la numeración y varias reglas cambian; aplica `## Entidad antes de artículo`; (b) **otra jurisdicción nacional** — cuando el usuario, el asunto o los hechos involucran un país distinto de México, reconocerlo y actuar en consecuencia; no aplicar silenciosamente doctrina mexicana a hechos de otra jurisdicción.
...
5. **Nunca producir una respuesta segura usando la ley de la jurisdicción equivocada.** Seguro-e-incorrecto es peor que incierto-y-marcado. Un abogado que te descubre aplicando el Código Civil de Jalisco a un inmueble en Querétaro, o el Código Civil Federal a un contrato regido por ley de Texas, deja de confiar en todo lo demás.
...
*Solo relevante para prácticas con múltiples clientes (práctica privada — despacho solo, pequeño, grande; notaría). Si eres jurídico interno de una sola empresa o administras solo tu propio patrimonio, esta sección está desactivada — los skills usan contexto a nivel de práctica automáticamente y `/civil-legal-mexico:matter-workspace` no es algo que necesites.*
...
Para civil-legal-mexico, un "asunto" es típicamente un contrato o expediente discreto: un arrendamiento (lado arrendador o arrendatario), una compraventa o promesa, una garantía, una sucesión, una reclamación de responsabilidad civil, o un convenio familiar.

Cuando los espacios de trabajo por asunto están habilitados, los skills trabajan en el contexto del asunto activo. Leen este CLAUDE.md para las reglas de práctica (entidades, lado habitual, postura de riesgo) y el `matter.md` del asunto para hechos específicos y anulaciones (incluida la entidad del inmueble cuando difiere de la principal). Los resultados se escriben en `~/.claude/plugins/config/claude-for-legal/civil-legal-mexico/matters/<asunto-slug>/`.

Cuando el contexto cruzado está desactivado (por defecto), un skill trabajando en el asunto A nunca lee archivos del asunto B. Administrar asuntos con `/civil-legal-mexico:matter-workspace new | list | switch | close | none`.

---

## Módulos activos

*Solo las secciones de módulos activos se escriben abajo. Los módulos inactivos se omiten por completo. En esta versión el único módulo disponible es Arrendamiento; los demás (contratos civiles, compraventa de inmuebles, garantías, plazos y prescripción, responsabilidad civil, sucesiones, familia) llegan en versiones posteriores.*

---

<!-- MÓDULO: Arrendamiento — activar para quien revisa, redacta o administra arrendamientos de inmuebles -->

## Arrendamiento

**Lado habitual:** [PLACEHOLDER — arrendador / arrendatario / ambos según el asunto / administrador de inmuebles]
**Destinos frecuentes:** [PLACEHOLDER — habitación / comercio / industria / oficina / agropecuario]
**Volumen:** [PLACEHOLDER — contratos por mes aproximados, o "ocasional"]
**Machote de casa:** [PLACEHOLDER — ruta del contrato modelo del despacho, o "ninguno"]
**Posiciones de playbook:**
- Depósito: [PLACEHOLDER — meses de renta y plazo de devolución que el despacho acepta]
- Incremento anual: [PLACEHOLDER — INPC / porcentaje fijo / tope]
- Fiador u obligado solidario: [PLACEHOLDER — siempre / solo comercial / según monto; ¿se exige extensión a prórrogas?]
- Pena por terminación anticipada: [PLACEHOLDER — meses de renta que el despacho considera aceptables por lado]
- Inscripción en RPP: [PLACEHOLDER — siempre arriba del umbral legal / solo si el cliente lo pide]
**Umbral de escalamiento:** [PLACEHOLDER — renta mensual o plazo a partir del cual el asunto se escala al socio]
**Registro de arrendamientos:** `~/.claude/plugins/config/claude-for-legal/civil-legal-mexico/arrendamientos/registro.yaml` *(vencimientos, ventanas de preferencia, incrementos — se llena al cerrar cada revisión si el usuario lo pide)*

---

*Re-ejecutar entrevista completa: `/civil-legal-mexico:cold-start-interview --redo`*
*Agregar un módulo: `/civil-legal-mexico:cold-start-interview --module [arrendamiento]`*
